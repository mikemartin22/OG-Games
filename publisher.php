...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		<title>OG Games - Publishers</title>
...
<div align="center" style="background-color: white; margin: 0px 375px 0px 375px">
	...
	<img alt="publisher button" src="buttons/publisher.JPG" />
...
<div id="accordion" style="background-color: white; margin: 0px 200px 0px 200px">
	...
	<div>
		...
		<?php
				...
				if(isset($_POST['insert']))
				{
					...
					elseif($error==0)
					{
						$myquery = mysql_query("SELECT MAX(PUB_ID) FROM PUBLISHER");
						$myrow = mysql_fetch_assoc($myquery);	
						$new_id = $myrow['MAX(PUB_ID)'];				
						$new_id = $new_id + 1;
							
						$query= mysql_query("INSERT INTO PUBLISHER VALUES ('$new_id','$name')");
						$viewquery= "SELECT * FROM PUBLISHER WHERE PUB_ID = '$new_id'";
							...
							echo "<center>";
								echo "<table border = '5' cellpadding = '5' style='font-size: 12px; border-color: black; border-style: outset; padding: 5px; text-align:center'>
									<tr bgcolor='white'>
										...
										echo"<p>Insertion successful. <br />The new data for Publisher ID ".$row['PUB_ID']." is displayed below.</p>";
										echo"<tr>";
										echo"<td style='width='400px''>" .$row['PUB_ID'] ."</td>";
										echo"<td>" .$row['NAME'] ."</td>";
									echo"</tr>";
								echo "</table>";
							echo"</center>";
			...
			Publisher: <input name="name" type="text" value="<?php echo $name; ?>"/><br /><br />			
	...
	<div>
		...
		<?php
			...
			if(isset($_POST['delete']))
			{
				...
				mysql_query("DELETE FROM PUBLISHER WHERE PUB_ID='$id'");
				
				echo '<script type="text/javascript">alert("Publisher '.$id.' has been deleted.");</script>';
			...
			if(isset($_POST['update']))
			{
				...
				$error = 0;
					...
					elseif($error==0)
					{
						...
						mysql_query("UPDATE PUBLISHER SET NAME ='$name' WHERE PUB_ID='$id'");

						echo '<script type="text/javascript">alert("Publisher '.$id.' has been updated.");</script>';
								...
								while($row=mysql_fetch_array($r))
								{
									...
									echo"<tr>";
										echo"<form method='post'>";
											echo"<td>$id</td>";
											echo"<input name='id' type='hidden' value='".$id."' />";
											echo"<td style='width='400px''><input name='name' type='text' value='".$name."' size='30' /></td>";
											echo"<td><input name='update' type='submit' value='' class='update' onclick='return confirm(\"Are you sure you would like to update this entry?\")'/></td>";
											echo"<td><input name='delete' type='submit' value='' class='delete' onclick='return confirm(\"Are you sure you would like to delete this entry?\")'/></td>";
										echo"</form>";
									echo"</tr>";
	...
	<form method="post">
		<p>
			Search Publisher by ID: <input name="id" type="text" maxlength="3" size="3"/>
								...
								echo "<center>";
									echo "<table border = '5' cellpadding = '5' style='font-size: 12px; border-color: black; border-style: outset; padding: 5px; text-align:center'>
										<tr bgcolor='white'>
											...
											$row=mysql_fetch_array($r);
											echo"<p>The data for Publisher ID ".$row['PUB_ID']." is displayed below.</p>";
											echo"<tr>";
											echo"<td>" .$row['NAME'] ."</td>";
										echo"</tr>";
									echo "</table>";
